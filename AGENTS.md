...
- Milk Diller availability synchronization is PostgreSQL-only and runs through `server/scripts/syncMilkDillerAvailability.cjs`. It must be deployed as a short-lived Railway Cron service every six hours (`0 */6 * * *`), not an in-process interval. The admin `Автоматична перевірка` switch controls `availabilitySyncEnabled`; enabling it also starts one immediate check. The admin panel and cron script share `server/services/supplierAvailabilitySync.cjs`.
...
- The public storefront silently reloads products once per minute while visible and on visibility return so completed supplier status changes appear without a full page reload. Do not use customer page views to scrape Milk Diller directly.
- In the public `supplier_order` catalog, supplier selection is an optional filter, not a gateway. Build category and subcategory navigation from all supplier-order products; if the chosen category is incompatible with the current supplier filter, clear that supplier filter instead of showing an empty catalog. Supplier-order cards and product details must always identify the supplier and state its minimum order, including when there is no minimum.
- Public product lists must sort orderable products before unavailable products. Apply this before catalog pagination so out-of-stock products fall onto the final pages, and use the same availability-first order in the related-products carousel on product details.
- Optional supplier sync tuning variables: `SUPPLIER_SYNC_USER_AGENT`, `SUPPLIER_SYNC_MASS_CHANGE_RATIO`, `MILKDILLER_FETCH_TIMEOUT_MS`, `MILKDILLER_FETCH_RETRIES`, `MILKDILLER_FETCH_CONCURRENCY`, and `MILKDILLER_MAX_PAGES`. Defaults are safe for the six-hour short-lived cron; `USE_POSTGRES=true` and `DATABASE_URL` are required.
